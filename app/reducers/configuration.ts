...
function configurationMigrations(
  ...
  logger.trace();
  logger.log('configuration:%j', oldConfiguration);
  ...
  if (JSON.stringify(oldConfiguration) !== JSON.stringify(migratedConfiguration)) {
    logger.info('Did migrate configuration');
    logger.log('migratedConfiguration:%j', migratedConfiguration);
  }
